...
const setTeachersElementsInCards = (cardElement, teachers) => {
  ...
  if (teachers.length == 0) {
    teachersElement.html("Card não contém professores vinculados.");
    return;
  }
...
const showDetailsCard = async (element) => {
  ...
  if (card.professores.length == 0) {
    modal.find("#professores").html("Card não contém professores vinculados.");
  } else {
